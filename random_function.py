...
def random_data_source(input_path, min, max) -> list:
    '''
    Select random data source from certain range, return a file list
    '''
    files = os.listdir(input_path)
    random_data_source_list = [files[i] for i in np.random.randint(0, len(files), size=int((np.random.rand()*(max-min)+min)))]
