...
class visualize:
    def __init__(self, data=None, data_path_list=None, configs='configs.ini', radius=2000, if_box=True,
        ...
        # read config.ini, get the candidates for the bonding points
        # get a list
    ...
    def update(self, frame: int):
        ...
        for data in self.data:
            ...
            # suspect, maybe can be a box, candidates
            if self.if_box:
                ...
                # draw the bonding_box
                self.bonding_box(x_max, x_min, y_max, y_min, z_max, z_min)

        self.ax.set_xlim3d([self.x0 - 2*self.radius, self.x0 + 2*self.radius]) # 画布大小
        self.ax.set_ylim3d([self.y0 - 2*self.radius, self.y0 + 2*self.radius])
        self.ax.set_zlim3d([0, self.radius + 2*self.z0])
    ...
    def bonding_box(self, x_max, x_min, y_max, y_min, z_max, z_min):
        """
        Draw the bonding_box if needed
        """
