...
class Human36mDataset(MocapDataset):
    ...
    # [ADDED] extrinsics parameter generation
    def __ext_gen(data):
        camera = dict()
        for subject, actions in data.items():
            camera[subject] = list()
            
            sel_point_0 = list(); sel_point_50 = list(); sel_point_100 = list()
            for action_name, positions in actions.items():
                #print(positions.shape)
                sel_point_0.append(positions[0,0])
                sel_point_50.append(positions[int(positions.shape[0]/2),0])
                sel_point_100.append(positions[-1,0])
            sel_point_0 = np.array(sel_point_0, dtype = np.float32)
            sel_point_50 = np.array(sel_point_50, dtype = np.float32)
            sel_point_100 = np.array(sel_point_100, dtype = np.float32)
            
            r_avg = (np.average(sel_point_0, 0) + np.average(sel_point_50, 0) + np.average(sel_point_100, 0))/3
            r_dif = Human36mDataset.__cal_dist(sel_point_0) + Human36mDataset.__cal_dist(sel_point_100)
            const_a = 1.5
            h = 1.0
            ...
            # r_cam.append(r_avg + np.array([r_dif * const_a * np.cos(theta + np.pi/2*1), r_dif * const_a * np.sin(theta + np.pi/2*1), h]))
    ...
    def __cal_dist(points):
        dist = 0
        for i in range(points.shape[0]):
            for j in range(i,points.shape[0]):
                r_diff = points[i] - points[j]
                dist += np.dot(r_diff, r_diff)
        return dist