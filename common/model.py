...
class Regression(nn.Module):
    ...
    def __init__(self, cam, inteval=243) -> None:
        super().__init__()
        self.cam = torch.tensor(cam).cuda()
        self.cx = self.cam[0]
        self.cy = self.cam[1]
        self.fx = self.cam[2]
        self.fy = self.cam[3]
        self.i = int((inteval-1)/2)
    ...
    def __call__(self, predicted_3d_pos, inputs_2d):
        ...
        self.inputs_2d += 1
        self.inputs_2d *= 1000/2
        
        self.inputs_2d[...,0] -= self.cx
        self.inputs_2d[...,0] /= self.fx
        self.inputs_2d[...,1] -= self.cy
        self.inputs_2d[...,1] /= self.fy
    ...
    def unit_matrix(self, index, frame):
        ...
        unit_A = torch.hstack([torch.vstack([-I,O,X]), torch.vstack([O,-I,Y])]).transpose(1,0)
        unit_b = torch.hstack([x-z*X, y-z*Y])

        return unit_A, unit_b

    def unit_solver(self, index, frame):
        A, b = self.unit_matrix(index, frame)
        t = torch.inverse(A.T @ A) @ A.T @ b
        return t
