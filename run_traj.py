...
# Replace the subject with sh36m
subjects_train = subjects_test = dataset.subjects()
...
def evaluate(test_generator, action=None, return_predictions=False):
    ...
    cam = [512.54150390625, 515.4514770507812, 1145.0494384765625, 1143.7811279296875]
    reg = Regression(cam)
    ...
    with torch.no_grad():
        ...
        for _, batch, batch_2d in test_generator.next_epoch():
            ...
            if torch.cuda.is_available():
                inputs_2d = inputs_2d.cuda()
            # Positional model
            predicted_3d_pos = model_pos(inputs_2d)  # pred_traj    
            init_3d_pos = init_model(inputs_2d) # pred_pos
            reg_traj = reg(init_3d_pos, inputs_2d)  # reg_traj
            gt_traj = predicted_3d_pos  # gt_traj
            ...
            if plot_3d:
                ...
                gt = gt_traj[0]
                pred = reg_traj[0].cpu().numpy()
                ax.plot3D(gt[:,0], gt[:,1], gt[:,2], 'blue')
                ax.plot3D(pred[:,0], pred[:,1], pred[:,2], 'red')
                ax.scatter3D(0,0,0)
                plt.savefig('traj3d.jpg')
            if plot_2d:
                from matplotlib import pyplot as plt
                plt.figure()
                gt = gt_traj[0]
                pred = reg_traj[0].cpu().numpy()
                x = np.linspace(0,gt.shape[0],gt.shape[0])
                plt.subplot(1,3,1)
                plt.plot(x, gt[:,0], 'blue')
                plt.plot(x, pred[:,0], 'red')
                plt.subplot(1,3,2)
                plt.plot(x, gt[:,1], 'blue')
                plt.plot(x, pred[:,1], 'red')
                plt.subplot(1,3,3)
                plt.plot(x, gt[:,2], 'blue')
                plt.plot(x, pred[:,2], 'red')
                plt.savefig('traj2d.jpg')

            # Test-time augmentation (if enabled)
            if test_generator.augment_enabled():
                # Undo flipping and take average with non-flipped version
                init_3d_pos[1, :, :, 0] *= -1
                init_3d_pos[1, :, joints_left + joints_right] = init_3d_pos[1, :, joints_right + joints_left]
                init_3d_pos = torch.mean(init_3d_pos, dim=0, keepdim=True)
            ...
            if torch.cuda.is_available():
                inputs_3d = inputs_3d.cuda()
            inputs_traj = inputs_3d[0:1, :, :1].clone()
            inputs_3d[:, :, 0] = 0
            if test_generator.augment_enabled():
                inputs_3d = inputs_3d[:1]
            
            
            inputs_3d += inputs_traj
            
            pred_traj = predicted_3d_pos.clone()
            init_3d_pos += predicted_3d_pos
            predicted_3d_pos = init_3d_pos
            ...
            in_traj = inputs_traj.to('cpu').detach().numpy()
            in_traj = in_traj.reshape(-1, in_traj.shape[-2], in_traj.shape[-1])
            pr_traj = pred_traj.to('cpu').detach().numpy()
            pr_traj = pr_traj.reshape(-1, pr_traj.shape[-2], pr_traj.shape[-1])
            #import pdb; pdb.set_trace()
            epoch_loss_3d_vel += inputs_3d.shape[0]*inputs_3d.shape[1] * n_mpjpe(torch.from_numpy(predicted_3d_pos-pr_traj).float().to('cuda').unsqueeze(0), 
                torch.from_numpy(inputs-in_traj).float().to('cuda').unsqueeze(0)).item()
