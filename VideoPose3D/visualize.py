...
class Visualization:
    ...
    def load_camera_view(self, args):
        try:
            dataset_orig = np.load('output/data_multi_output_' + args.dataset + '.npz', allow_pickle=True)["posetions_2d"].item()
            self.dataset = dataset_orig[0]
    ...
    def load_prediction(self, args):
        ...
        try:
            ...
            self.count_key_list = list(prediction_orig.keys())
            if args.sample is not None: 
                sample_key = int(args.sample); 
            else: 
                sample_key = random.choice( self.count_key_list )

            self.prediction = prediction_orig[self.count_key_list[sample_key]]
            self.pos_pred = self.prediction["pose_pred"][0].transpose(0,1).transpose(0,2)
            self.pos_pred = torch.mul(self.pos_pred,torch.tensor(1e3))
            #self.pos_pred = torch.add(self.pos_pred, self.prediction["T"][0])
            self.pos_pred[:,:,:,1] = - self.pos_pred[:,:,:,1]
            self.pos_pred = self.pos_pred.transpose(0,2).transpose(0,1)
        except KeyError:
            print('Sample does not exist! Please input right sample number')
    ...
    def get_datainfo(self, args):
        ...
        try:
            ...
            self.x = self.dataset[self.view_key_list[0]]['pose_2d'].shape[1] - self.prediction["receptive_field"]
        ...
        self.center_p = torch.mean(self.pos_pred[:,0,10,:],axis=0).cpu().detach().numpy()
        print(self.center_p)
    ...
    def plotPred(self, frame):
        if self.ifcomp:
            prediction_data = self.pos_pred; k = 0
            for person in prediction_data:
                for i in self.skeleton:
                    x = torch.stack((person[frame, i[0], 0], person[frame, i[1], 0]), 0).cpu().detach().numpy()
                    z = torch.stack((person[frame, i[0], 1], person[frame, i[1], 1]), 0).cpu().detach().numpy()
                    y = torch.stack((person[frame, i[0], 2], person[frame, i[1], 2]), 0).cpu().detach().numpy()
                    self.ax[self.view_key_list[0]][0].plot3D(x, y, z, lw=2, c=self.color[-1], alpha = 0.8); 
                k+=1
            ...
            for person in prediction_data:
                for i in self.skeleton:
                    ...
                    z = torch.stack((person[frame, i[0], 1], person[frame, i[1], 1]), 0).cpu().detach().numpy()
                    y = torch.stack((person[frame, i[0], 2], person[frame, i[1], 2]), 0).cpu().detach().numpy()
                    self.bx.plot3D(x, y, z, lw=2, c=self.color[k], alpha = 0.8); 
                k+=1

            self.bx.set_xlim3d([self.center_p[0] - 2*self.radius_m, self.center_p[0] + 2*self.radius_m]) # 画布大小
            self.bx.set_ylim3d([self.center_p[2] - 2*self.radius_m, self.center_p[2] + 2*self.radius_m])
            self.bx.set_zlim3d([-self.radius_m/2 + self.center_p[1], self.radius_m/2 + self.center_p[1]])
    ...
    def updater(self, frame):
        ...
        if self.ifcomp:
            for key in self.view_key_list:
                ...
                pass
            Visualization.plotPred(self, frame)
        else:
            Visualization.plotPred(self, frame)
